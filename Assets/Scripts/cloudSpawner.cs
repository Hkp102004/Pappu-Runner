...
public class cloudSpawner : MonoBehaviour
{
    ...
    void spawning()
    {
        ...
        if(time>=rate)
        {
            Instantiate(cloud, new Vector3(transform.position.x, player.position.y + UnityEngine.Random.Range(pivoty,pivoty+2) , transform.position.z), Quaternion.identity);
            time=0;
        }
        else
        {
